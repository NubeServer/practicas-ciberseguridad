# Mis Prácticas de Ciberseguridad

Bienvenido a mi repositorio de Ciberseguridad. Aquí documentaré mis avances y mis prácticas semanales

## Práctica 1: Compartir archivos entre kali-linux y Windows 11(host) usando VMWARE
- Autor: NubeServer
- Fecha: 15-Agosto 2026
- **Documento completo**: [Descargar PDF aquí](Carpetas_Compartidas_VMWARE.pdf)   
🔒 **Integridad del archivo (SHA256)**:
`2596A06900384CC422AACD5C3B1A78CCB47B7D5180EA3AD991232C7E1FE43FCA`

## Práctica 2: Aseguramiento de documentos con la Triada
- Autor: NubeServer
- Fecha: 17/08/2026
-  **Documento completo**: [Descargar PDF aquí](Practica_2_Aseguramiento_de_documentos_con_la_TRIADA.pdf) 
🔒 **Integridad del archivo (SHA256)**:
`3A9CCFE64AE81D46748E2B307A3AE0EB260D938C96140F61D225D184BF53A236`

## Práctica 2: Configurar OpenVPN desde VMWARE a Tryhackme
